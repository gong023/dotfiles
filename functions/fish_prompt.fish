...
function prompt_dir -d "Display the current directory"
  #set -l gopathtmp (string replace -r '^'"$HOME"'($|/)' '~$1' $GOPATH)
  #set -l short (string replace -r $gopathtmp '~(GOPATH)/' (prompt_pwd))
  #set -l short (string replace -r '/Dropbox/program' '/d/p' $short)
  #set -l short (basename (prompt_pwd))
  prompt_segment blue black (basename (prompt_pwd))
end

function prompt_cluster -d "Display current GKE cluster"
  set -l cluster (kubectl config current-context | cut -f 4 -d '_')
  prompt_segment red black $cluster
end

function prompt_cluster_namespace -d "Display current cluster namespace"
  set -l cluster (kubectl config current-context)
  set -l namespace (kubectl config view -o=jsonpath="{.contexts[?(@.name==\"$cluster\")].context.namespace}")
  prompt_segment red black $namespace
end
